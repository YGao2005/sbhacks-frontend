...
const performSemanticSearch = async (mainQuery: string) => {
  ...
  const handleSaveToCollection = async () => {
    if (!collectionId || selectedPapers.size === 0) {
      toast({
        title: "Error",
        description: "No collection ID provided or no papers selected",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      // Get the collection document reference
      ...
      // Get the papers to add from the search results
      ...
      // Update the collection with the new papers and increment papersCount
      ...
      toast({
        title: "Success",
        description: `Added ${papersToAdd.length} papers to your collection`,
      });
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to save papers to collection",
        variant: "destructive",
      });
    } finally {
